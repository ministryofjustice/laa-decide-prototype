...
router.get('/decision-start', function(req, res) {
  ...
  res.redirect('/static/overall-decision');
});
...
router.post('/decision-check', function(request, response) {
    ...
    if (decisionCheck == "refuse") {
        response.redirect("/static/refuse-reason")
    } else {
      response.redirect("/static/grant-certificate-date")
    }
});
...
router.post('/grant-date-submit', function(request, response) {
    ...
    response.redirect("/static/check-answers")
});
...
router.post('/refuse-decision-submit', function(request, response) {
    ...
    if (application) {
        ...
        request.session.data['app-history'][decisionReference].push({
            ...
            versionLink: '/static/application/' + decisionReference
        })
...
const SEEDED_HISTORY = {
  ...
  'L-REFUS-ED1A': [
    ...
    { timestamp: '01 Mar 2026 10:30', action: 'Client first name changed', caseworker: 'Michelle Foster', details: 'From: Patriciase\nTo: Patricia', justification: 'Name updated following request to correct spelling mistake. Verification of legal documentation completed.' },
  ...
  if (req.session.data['app-history'] && req.session.data['app-history'][reference]) {
    ...
    historyEvents = req.session.data['app-history'][reference].map((event, index) => {
      ...
      return {
        ...
        title: action,
        versionLink: !isLastEvent ? `/static/application/${reference}?viewVersion=${index}${tabAnchor}` : null,
...
router.post('/change/:reference/:field', function(req, res) {
  ...
  res.redirect(`/static/change/${reference}/${field}/confirm`);
});
...
router.post('/change/:reference/:field/confirm', function(req, res) {
  ...
  res.redirect(`/static/application/${reference}#people`);
});
