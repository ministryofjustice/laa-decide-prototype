...
router.post('/send-back-check', function(request, response) {
    ...
    if (sendbackCheck == "rfi") {
        response.redirect("/v5/my-applications-rfi")
    }  else if (sendbackCheck == "withdraw") {
      response.redirect("/v5/my-applications-withdraw")
    }  else {
      response.redirect("/v5/my-applications-rejected")
    }
});
...
router.post('/merits-check', function(request, response) {
    ...
    if (meritsCheck == "granted") {
        response.redirect("/v5/merits-assessment-emergency-costs")
    } else {
      response.redirect("/v5/merits-assessment-substantive")
    }
});
...
router.post('/merits-check2', function(request, response) {
    ...
    if (meritsCheck2 == "granted") {
        response.redirect("/v5/merits-assessment-substantive-costs")
    } else {
      response.redirect("/v5/decision-communication")
    }
});
...
router.post('/decision-check', function(request, response) {
    ...
    if (decisionCheck == "refuse") {
        response.redirect("/v5/overall-decision-error")
    }  else {
      response.redirect("/v5/confirmation-screen")
    }
});
